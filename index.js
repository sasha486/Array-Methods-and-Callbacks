...
function getFinals(data) {
  const finals = fifaData.filter((team) => team.Stage === "Final");
  return finals;
}
...
function getYears(array, cb) {
  const finalYears = cb(array);
  return finalYears.map((years) => years.Year);
}
...
function getWinners(array, cb) {
  const winners = array.map((wins) => {
    return wins["Home Team Goals"] > wins["Away Team Goals"]
      ? wins["Home Team Name"]
      : wins["Away Team Name"];
  });
  return winners;
